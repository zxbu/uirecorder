    ...
    inquirer.prompt(questions).then(function(answers){
        ...
        function onReady(serverPort){
            ...
            setTimeout(function(){
                newChromeBrowser({wdPort: wdPort, hosts: hosts, wdPacUrl: wdPacUrl, isRecorder: true, debug: debug}, function*(browser){
                    ...
                    // todo 开始录制
                    var res = request('POST', process.env.UIRECORDER_MOCK_API + '/uirecorder-mock-manager/beginRecorder');
                    recordId = res.getBody('utf8');
        ...
        function onEnd(bSaveFile){
            recorderBrowser.close(function(){
                ...
                console.log('');
                // todo 结束录制
                var res = request('POST', process.env.UIRECORDER_MOCK_API + '/uirecorder-mock-manager/endRecorder');
                res.getBody('utf8');
                console.log(__('end_recorder').green, recordId);
                if(bSaveFile){
                    saveTestFile();
                }
