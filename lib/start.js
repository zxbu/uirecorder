    ...
    inquirer.prompt(questions).then(function(answers){
        ...
        function onReady(serverPort){
            ...
            setTimeout(function(){
                newChromeBrowser({wdPort: wdPort, hosts: hosts, wdPacUrl: wdPacUrl, isRecorder: true, debug: debug}, function*(browser){
                    ...
                    // todo 开始录制
                    request(process.env.UIRECORDER_MOCK_API + '/uirecorder-mock-manager/beginRecorder', function (error, response, body) {
                        if (!error && response.statusCode === 200) {
                            recordId = body;
                            console.log(__('begin_recorder').green, recordId);
                        } else {
                            console.log(__('begin_recorder_failed').green, response.statusCode);
                        }
                    });
        ...
        function onEnd(bSaveFile){
            recorderBrowser.close(function(){
                // todo 结束录制
                request(process.env.UIRECORDER_MOCK_API + '/uirecorder-mock-manager/endRecorder', function (error, response, body) {
                    if (!error && response.statusCode === 200) {
                        recordId = body;
                        console.log(__('end_recorder').green);
                    } else {
                        console.log(__('end_recorder_failed').green, response.statusCode);
                    }
                });
